...
浏览器访问 `http://127.0.0.1:8787/`（仅本机可访问），统一完成：状态查看、虚拟打印机检测/修复、打印校准唛、切换打印方向（顺/逆时针，即时生效）、打开各目录、查看最近处理与运行日志。页面文案与配色在 `web\操作页面.html` 中（文件顶部有修改指引）。
...
- 先打印一张真实水洗唛试跑，核对方向与位置；如需对比，双击 `打印校准唛.cmd` 会打出两版（顺时针/逆时针）各一张，选与日常一致的那版，然后在操作页面「打印方向」里点选（即时生效；详见说明书第 4 节）。
...
- 不打印关键词：命中 布草 / 洗涤服务 / 窗帘 / 窗纱（可配置）的标签自动跳过打印，仅保留归档与预览；
