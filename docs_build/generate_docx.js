...
function bodyChildren() {
  ...
  out.push(makeTable(["区域 / 按钮", "功能"], [
    ...
    ["打印方向", "切换条码旋转方向（顺时针 / 逆时针），点击后立即生效并自动保存"],
  ...
  out.push(p("不同门店的标签机安装方向、耗材方向可能存在差异，工具提供两版校准样张：双击「打印校准唛.cmd」（或操作页面「打印校准唛（两版）」按钮），会连续打印两张测试标签——第 1 版为顺时针旋转、第 2 版为逆时针旋转。选择与门店日常输出一致的方向后，在操作页面「打印方向」里点选即可（即时生效）。"));
  out.push(p("默认值：rotate_dir 为 cw（顺时针）。如需改为逆时针：在操作页面「打印方向」卡片点选「逆时针 90°」即可（立即生效并自动保存，无需重启）；也可将 config.json 中 rotate_dir 改为 ccw（重启后生效）。"));
  ...
  out.push(makeTable(["问题", "处理"], [
    ...
    ["标签打印出来方向不对？", "在操作页面「打印方向」卡片切换（立即生效，无需重启）；详见 4.1 节。"],
  ...
  out.push(p("软件自带一个本地操作页面（浏览器打开，地址 http://127.0.0.1:8787/，仅本机可访问），它是日常唯一的操作界面：查看运行状态与捕获情况、检测/修复虚拟打印机、打印校准唛、切换打印方向（顺 / 逆时针，即时生效）、打开各目录、查看最近处理与运行日志。"));
