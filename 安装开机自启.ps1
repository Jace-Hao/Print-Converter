...
if (-not (Test-Path $pythonw)) { $pythonw = Join-Path $env:LOCALAPPDATA 'Python\bin\pythonw.exe' }
if (-not (Test-Path $pythonw)) { Write-Host "找不到 pythonw.exe：$pythonw"; if (-not $Silent) { Read-Host '按回车退出' }; exit 1 }
